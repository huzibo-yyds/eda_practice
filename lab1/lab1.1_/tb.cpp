...
int main() {
    ...
    sync_circuit(input_stream, output_stream);
    ...
    // 清空流避免残留数据
    while (!output_stream.empty()) output_stream.read(result);
    ...
    printf("\n==== Test Summary ====\n");
    int error_count = 0;
    if (success_flags[0] != 1) error_count++;
    if (success_flags[1] != 1) error_count++;
